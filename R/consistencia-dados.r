...
scatter_plot_model <- function(dt_ref, dt_fonte_orig, dt_fonte_ajust, model) {
  # Merge para alinhamento
  dt_plot <- merge(dt_ref, dt_fonte_orig, by = "data_hora_observacao", all = FALSE)
  dt_plot <- merge(dt_plot, dt_fonte_ajust, by = "data_hora_observacao", all = FALSE)

  a <- model[["a"]]
  b <- model[["b"]]

  # dt_plot <- dt_plot[complete.cases(dt_plot)]

  if (nrow(dt_plot) > 0) {
    plot(
      dt_plot$valor_orig, dt_plot$valor_ref,
      xlab = paste("Valor Original)"),
      ylab = paste("Valor Referencia/Ajustado"),
      main = paste("Ajuste Ajustado"),
      pch = 16, col = "blue",
      cex = 1.2,
      xlim = c(0, max(dt_plot$valor_ajust * 1.1, na.rm = T)),
      ylim = c(0, max(dt_plot$valor_ref * 1.1, na.rm = T))
    )

    points(
      dt_plot$valor_ajust, dt_plot$valor_ref,
      pch = 17, col = "red", cex = 1.2
    )

    points(
      dt_plot$valor_orig, dt_plot$valor_ajust,
      pch = 17, col = "yellow", cex = 1.2
    )


    abline(a = b, b = a, col = "darkgreen", lwd = 2)
