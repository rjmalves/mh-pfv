...
test_that("predict_main partial failure: 1 of 3 plants fails", {
    temp_output <- withr::local_tempdir()
    config <- gen_config(
        mode = "predict",
    ...
    report_files <- list.files(temp_output, "^health-.*\\.json$",
        full.names = TRUE)
...
test_that("predict_main all plants fail", {
    temp_output <- withr::local_tempdir()
    config <- gen_config(
        mode = "predict",
    ...
    mockery::stub(predict_main, "adicionar_passo_previsao",
        function(x, ...) x)
    mockery::stub(predict_main, "processar_usina",
